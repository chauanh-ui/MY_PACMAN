...
AddedGhost::AddedGhost() : Ghost(Green, EntityType::addedGhost) {

	modIsAddedGhost(true);
	this->ScatterTarget.ModCoords(26 * BlockSize24 + BlockSize24 / 2, 35 * BlockSize24 + BlockSize24 / 2);
	this->Home.ModCoords(11 * BlockSize24, 17 * BlockSize24);

}

void AddedGhost::CalculateTarget(Entity mPac, Position mBlinky) {
	short x = mPac.GetX();
	short y = mPac.GetY();
...
void AddedGhost::UpdatePos(unsigned char ActualBoard[], Pac& mPac, Position mBlinky, bool TimedStatus) {
